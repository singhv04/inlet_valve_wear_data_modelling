# # generate_inlet_valve_data.py (CONTROLLED WEAR DISTRIBUTION VERSION)
...
#     return velocity + np.random.normal(0, 0.05 * velocity)
...
#     wear *= np.random.normal(loc=1.0, scale=0.10)
#     return wear
...
#     # Define wear target ranges
#     wear_targets = np.linspace(0.3, 2.9, wear_bins)

#     for wear_center in tqdm(wear_targets):
#         for _ in range(samples_per_bin):
#             # Random base parameters
#             pressure = np.random.uniform(150, 250)
#             rpm = np.random.normal(1600, 300)
#             seat_angle = np.random.uniform(20, 45)
#             insert_angle = seat_angle + np.random.uniform(-2, 2)
#             diameter = np.random.uniform(35, 55)
#             duration = np.random.uniform(200, 3000)
#             lubrication_index = np.random.uniform(0.5, 1.0)
#             face_width = np.random.uniform(1.5, 4.0)
...
#             wear = compute_wear(pressure, velocity, duration, temp_mult, material_mod,
#                                 mismatch, hardness, face_width, coeff_mod)

#             # Force wear near target range with exponential decay
#             wear *= wear_center / (wear + 1e-6)  # scale towards target
#             wear += np.random.normal(0, 0.05)

#             # Soft bounds
#             wear = max(0.1, min(wear, 3.0))
...
#     df = generate_synthetic_data(n_samples=100_000)
#     df.to_csv("synthetic_inlet_valve_data.csv", index=False)
...
#     plt.grid(True)
#     plt.tight_layout()
#     plt.savefig("inlet_wear_distribution_histogram.png")
#     plt.show()
#     print("📊 Histogram saved as wear_distribution_histogram.png")
